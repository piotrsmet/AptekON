...
export const getRezerwacjeByApteka = async (req, res) => {
	try {
		...
		if (owner === 'true' && apteka_id) {
			const rezerwacje = await db.all(
				`
        SELECT r.id, r.user_id, r.lek_id, r.ilosc, r.data_rezerwacji, r.data_wygasniecia,
               l.nazwa as lek_nazwa, l.moc as lek_moc,
               u.email as uzytkownik_email
        FROM rezerwacje r
        LEFT JOIN leki l ON r.lek_id = l.id
        LEFT JOIN uzytkownicy u ON r.user_id = u.id
        WHERE r.apteka_id = ? AND r.data_wygasniecia > datetime('now')
        ORDER BY r.data_rezerwacji DESC
      `,
				[apteka_id]
			)
		...
		if (!apteka_id || !user_id) {
			return res
				.status(400)
				.json({ error: 'Brakuje apteka_id lub user_id' })
		}
